...
const validateForm = (data: Record<string, string>, rules: ValidationRules): FormErrors => {
...
const SHIPPING_RULES: ValidationRules = {
...
const PAYMENT_RULES: ValidationRules = {
...
export default function CheckoutPage() {
  ...
  const sanitize = (str: string) => {
    ...
    return str.replace(/[<>"'&]/g, (c) => ({ '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;', '&': '&amp;' }[c as keyof typeof map] || c));
  };
  const map = { '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;', '&': '&amp;' };

  const handleNextStep = (e: React.FormEvent) => {
    e.preventDefault();
    const errors = validateForm(shippingInfo, SHIPPING_RULES);
  ...
  const handlePlaceOrder = (e: React.FormEvent) => {
    e.preventDefault();
    const errors = validateForm(paymentInfo, PAYMENT_RULES);
    setFormErrors(errors);
    if (Object.keys(errors).length > 0) return;
    ...
    if (!orderNumberRef.current) {
      ...
      const orderId = uuid.replace(/-/g, "").substring(0, 12).toUpperCase();
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 lg:px-8 py-20 flex flex-col items-center justify-center text-center animate-fade-in">
        ...
        <p className="text-text-secondary mb-8">
          Thank you, {sanitize(shippingInfo.firstName) || "Customer"}. Your order #{orderNumber || "LX-XXXXXXXXXXXX"} has been placed successfully.
