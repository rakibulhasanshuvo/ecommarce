...
export default function CartPage() {
  ...
  const handleApplyDiscountWithFeedback = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (!isValid) {
      const activeCodes = discountCodes.filter(d => d.active).map(d => d.code);
      const suggestions = activeCodes.slice(0, 3);

      let suggestionText = "";
      if (suggestions.length > 0) {
        const formatter = new Intl.ListFormat('en', { style: 'long', type: 'disjunction' });
        suggestionText = `. Try ${formatter.format(suggestions)}`;
      }

      setDiscountError(`Invalid discount code${suggestionText}.`);
